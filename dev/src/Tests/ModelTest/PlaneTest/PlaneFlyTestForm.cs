...
{
    public partial class PlaneFlyTestForm : Form
    {
        //Plane plane;
        private Level level;

        public PlaneFlyTestForm()
        {
            InitializeComponent();
        ...
        {
            //level.UserPlane.Move(1);
            level.Update(timer1.Interval);
            pictureBox1.Refresh();
            //Text = plane.MovementVector.ToString();
            ...
            Text += " Prêdkoœæ : " + level.UserPlane.MovementVector.EuclidesLength;
        }

        private void PlaneFlyTestForm_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Up:
