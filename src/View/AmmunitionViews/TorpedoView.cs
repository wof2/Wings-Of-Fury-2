...
namespace Wof.View
{
    internal class TorpedoView : AmmunitionView, IDisposable, WaterTrailGenerator
    {
            ...
            get
            {
                return
                    (!EngineConfig.LowDetails && Environment.TickCount - lastWaterTrailTime > 50 &&
                     this.ammunition.State == MissileState.Intact && ammunitionNode.Position.y < 0.5f);
            }
        }
        ...
        protected LevelView levelViewRef;

        public LevelView LevelViewRef
        {
            get { return levelViewRef; }
        }


        protected static Stack<TorpedoView> torpedoAvailablePool;
        protected static Dictionary<Ammunition, TorpedoView> torpedoUsedPool;
        ...
        {
            while (torpedoAvailablePool.Count > 0)
            {
                torpedoAvailablePool.Pop().Dispose();
            }
            torpedoAvailablePool.Clear();

            Dictionary<Ammunition, TorpedoView>.Enumerator e = torpedoUsedPool.GetEnumerator();
            while (e.MoveNext())
            {
                e.Current.Value.Dispose();
            }
            torpedoUsedPool.Clear();
        }
        ...
        public override void refreshPosition()
        {
            ...
            if ((this.ammunition as Torpedo).IsInWater && IsReadyForLastWaterTrail)
            {
                SceneNode splashNode = levelViewRef.getSplashNode();
                if (splashNode != null) // koniec poola
                {
                    NodeAnimation.NodeAnimation wna;
                    Vector3 position = new Vector3(AmmunitionNode.Position.x, 0.25f, 0);

                    wna =
                        EffectsManager.Singleton.RectangularEffect(sceneMgr, splashNode, "TorpedoWaterTrail",
        ...
        {
            torpedoAnimation.updateTime(timeSinceLastFrameUpdate);
            torpedoAnimation.animate();
        }
