...
namespace Wof.Controller.Screens
{
    internal class VideoModeScreen : AbstractOptionsScreen
    {
        ...
        private List<object> FilterPossibleOptions(List<String> available)
        {
            ...
            // do najwiekszej, wiec wystarczy, ze znajde wartosc graniczna
            foreach (String option in available)
            {
                string[] opts = option.Trim().Split(new char[] {' '});
                if (opts.Length > 0)
                {
                    int size = Int32.Parse(opts[0]);
                    if (size < 800)
                    {
                        mayCopy = false;
                    }
                    ...
                    {
                        int size1 = Int32.Parse(opts[0]);
                        int size2 = Int32.Parse(opts[2]);

                        // obs³uga tylko: 4/3
                        /* if ((float)size1 / (float)size2 == (float)4 / (float)3)
                        {
                            mayCopy = true;
                        }
                        else
                        ...
                        mayCopy = true; // obs³uga wszystkich trybów
                    }
                }
                else
                {
